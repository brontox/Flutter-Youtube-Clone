import 'package:youtube_ui/model.dart';

final List<YoutubeVideo> videos = [
  new YoutubeVideo(
    thumbnail:
        "https://preview.redd.it/1spsf7jvbsn11.png?width=1024&s=6be6c5334438473ecb960ae4ccf7f83766e59800",
    avatar:
        "https://ichef.bbci.co.uk/news/660/cpsprodpb/BC3A/production/_92868184_gettyimages-494848232.jpg",
    channel: "PewDiePie",
    title: "How I got so popular on youtube",
    views: 23443432,
    date: DateTime.utc(2018, 2, 4),
  ),
  new YoutubeVideo(
    thumbnail:
        "http://www.fs17.co/images/97450/FS17-valtra-T-Series-Cow-Edition-V-2.0.jpg",
    avatar:
        "https://yt3.ggpht.com/a-/AN66SAxPfKnfHAnAs0rOqaSwINOxDYJsyj-gPBP0OQ=s900-mo-c-c0xffffffff-rj-k-no",
    channel: "T-Series",
    title: "best GAME plays for kids under 13",
    views: 234,
    date: DateTime.utc(2018, 10, 16),
  ),
  new YoutubeVideo(
    thumbnail: "https://i.ytimg.com/vi/kffacxfA7G4/maxresdefault.jpg",
    avatar:
        "https://cdn3-www.musicfeeds.com.au/assets/uploads/justin-bieber-2016-press-pic-supplied-671x377.jpg",
    channel: "Justine Bieber",
    title: "Baby Baby Official - JUSTINE BIEBER",
    views: 10323233748,
    date: DateTime.utc(2013, 12, 1),
  ),
  new YoutubeVideo(
    thumbnail: "https://i.ytimg.com/vi/CNkxLavH0Lc/maxresdefault.jpg",
    avatar:
        "https://static1.squarespace.com/static/56a44a68cbced66ef85f1331/56a8cc40df40f3b3891c89a0/56a8d60fa976afa6650bc5e5/1453906043205/MIKE+TYSON.jpg",
    channel: "Boxing Legends",
    title: "Most Feared Current Boxers",
    views: 2398383,
    date: DateTime.utc(2018, 1, 1),
  ),
  new YoutubeVideo(
    thumbnail: "https://img.youtube.com/vi/e1Qf753lklY/maxresdefault.jpg",
    avatar:
        "https://pbs.twimg.com/profile_images/762515266525761536/HHvPiuIl.jpg",
    channel: "MMA World",
    title: "Dana White announced DC Vs Stipe Miocic",
    views: 23444,
    date: DateTime.utc(2017, 12, 1),
  ),
  new YoutubeVideo(
    thumbnail: "https://i.ytimg.com/vi/DqVPbEcWtA0/maxresdefault.jpg",
    avatar:
        "https://i.pinimg.com/736x/33/bb/2a/33bb2aaeba7be212b2991fc60548905d--cold-fusion-rossi.jpg",
    channel: "ColdFusion",
    title: "How Apple became a trillion dollar company",
    views: 234434,
    date: DateTime.utc(2018, 4, 1),
  ),
];
